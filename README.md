...
**Версия `14.6.1`** · [история изменений](CHANGELOG.md) · [правила системы](CLAUDE.md) · [решения и почему так](DECISIONS.md) · [как менять токены](tooling/README.md)
...
| Кейс системы | `site/case-r14.html` | Как собиралась система: аналитика, работа над ошибками, решения и автопроверки. Живые демонстрации на ядре R14, анимации проигрываются сами |
...
| Схема зала | `site/Схема зала - десктоп (для теста).html` | Новая схема зала для десктопа, тестовая сборка. Работает офлайн |
...
| Передача фронтенду | `site/handoff/frontend-changes.html` | Что изменилось в cookie-баннере и на странице партнёров: стало, рядом прежний вариант, все состояния. Статичный файл для импорта в Figma (html.to.design) |
| Письма | `emails/` | Транзакционные письма: подтверждение оплаты, напоминание о спектакле, все шаблоны в одном файле |
...
Витрина со ссылками на все страницы открыта на GitHub Pages: **https://hitch-npc.github.io/tts-design-system/** — обновляется после каждого push в `main`. Без интернета — скачайте репозиторий и откройте [`index.html`](index.html).
...
npm run check:strict  # то же, что check, но сырой #hex вне :root — ошибка (так гоняет CI)
```
